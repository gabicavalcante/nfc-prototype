...
def get(idp_endpoint, sp_target, login, debug=False):
    ...
    #relay_state.text = "bug"
    sp_package[0][0] = relay_state # etree.fromstring("bug") # relay_state 
    ...
    r = response.read()
    print(r)
    soup = BeautifulSoup(r, 'lxml') # Parse the HTML as a string
...
def main():
    ...
    usage = "usage: %prog [options] IdP_tag SP_tag login"
    ...
    if idp_tag not in IDP_ENDPOINTS:
    ...
    sp_endpoint = SP_ENDPOINTS[target]
    
    # get the target from the SP using ECP exchange
    get(idp_endpoint, sp_endpoint, login, options.debug)
